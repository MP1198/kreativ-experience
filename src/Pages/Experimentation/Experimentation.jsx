...
const Experimentation2 = ({ isDown }) => {
  ...
  useEffect(() => {
    const lettres = Array.from(
      document.querySelectorAll("h1 .experimentation-lettres")
    );
    let tireEl;
    ...
    const timerExplosion = async () => {
      timeoutExplosion.current = await awaitAnimation(
        setTimeout(() => {
          explosion();
          posRandomCible();
          fadeOutCible();
    ...
    function calculerCouleur(e) {
      ...
      const distance = Math.sqrt(
        Math.pow(sourisX - centreX, 2) + Math.pow(sourisY - centreY, 2)
      );
    ...
    const creationTire = (e) => {
      ...
      tireEl.style.left = centerX + "px";
      tireEl.style.top = centerY + "px";
  ...
  return (
    <div className="experimentation-container" ref={composantsRefs}>
      ...
      <h1 className="experimentation-titre" ref={composantsRefs}>
        <p className="experimentation-lettres" ref={composantsRefs}>
          E
        </p>
        <p className="experimentation-lettres" ref={composantsRefs}>
          x
      ...
      <Instruction
        texte={"Cliquez afin de trouver le point qui reformera le mot"}
        delais={3000}
        delaisOut={20000}
        couleur={"rouge"}
      />
